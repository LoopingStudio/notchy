      ...
      if Defaults[.enableGestures] {
        ...
        Slider(value: $gestureSensitivity, in: 100...300, step: 100) {
          HStack {
            ...
            Spacer()
            Text(
              Defaults[.gestureSensitivity] == 100 ? "High" : Defaults[.gestureSensitivity] == 200 ? "Medium" : "Low"
            )
            ...
            HStack {
              ...
              HStack(spacing: 6) {
                Circle()
                  ...
                  .foregroundColor(
                    isExtensionRunning(item.bundleIdentifier) ? .green : item.status == .disabled ? .gray : .red
                  )
                ...
                Text(
                  isExtensionRunning(item.bundleIdentifier)
                    ? "Running" : item.status == .disabled ? "Disabled" : "Stopped"
                )
              ...
              Menu(
                content: {
                  ...
                  Button("Disable") {
                    if let ext = NSWorkspace.shared.runningApplications.first(where: {
      ...
      Section {
        List {
          ForEach(customVisualizers, id: \.self) { visualizer in
            HStack {
              LottieView(
                state: LUStateData(type: .loadedFrom(visualizer.url), speed: visualizer.speed, loopMode: .loop)
              )
            ...
            .background(
              selectedListVisualizer != nil
                ? selectedListVisualizer == visualizer ? Defaults[.accentColor] : Color.clear : Color.clear,
