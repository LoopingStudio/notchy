...
struct NotchContentView: View {
      ...
      if !vm.firstLaunch {
        HStack(spacing: 14) {
          ...
          if !vm.expandingView.show {
                  ...
                  Image(nsImage: musicManager.albumArt)
                    ...
                    .frame(
                      width: vm.notchState == .open
                        ? vm.musicPlayerSizes.image.size.opened.width : vm.musicPlayerSizes.image.size.closed.width,
                      height: vm.notchState == .open
                        ? vm.musicPlayerSizes.image.size.opened.height : vm.musicPlayerSizes.image.size.closed.height
                    )
                    .cornerRadius(
                      vm.notchState == .open
                        ? vm.musicPlayerSizes.image.cornerRadius.opened.inset!
                        : vm.musicPlayerSizes.image.cornerRadius.closed.inset!
                    )
                    .scaledToFit()
            ...
            if musicManager.isPlayerIdle == true && vm.notchState == .closed && !vm.expandingView.show
            ...
            if vm.notchState == .closed && vm.expandingView.show {
              if vm.expandingView.type == .battery {
                BoringBatteryView(
                  ...
                  batteryWidth: 30, isInLowPowerMode: batteryModel.isInLowPowerMode)
              } else {
  ...
  func calculateFrameWidthforNotchContent() -> CGFloat? {
    ...
    let chargingInfoWidth: CGFloat =
      vm.expandingView.show
      ? ((vm.expandingView.type == .download ? downloadSneakSize.width : batterySneakSize.width) + 10) : 0
    let musicPlayingWidth: CGFloat =
      (!vm.firstLaunch && !vm.expandingView.show
        && (musicManager.isPlaying || (musicManager.isPlayerIdle ? Defaults[.showNotHumanFace] : true))) ? 60 : -15
    ...
    return vm.notchState == .open
      ? vm.musicPlayerSizes.player.size.opened.width! + 30 : dynamicWidth + (vm.sneakPeek.show ? -12 : 0)
  }
